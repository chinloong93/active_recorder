...
class Record
  ...
  def initialize(name)
    # Validate name
    fail ArgumentError.new('Name cannot be empty!') if name.empty?
    fail ArgumentError.new('Name cannot be nil!') if name.nil?
  ...
  def add_column(col, type)
    ...
    fail ArgumentError.new('Invalid ActiveRecord datatype!') unless VALID_TYPES.include?(type)
